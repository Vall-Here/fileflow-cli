...
export const SUPPORTED_FORMATS = ['csv', 'json', 'xlsx', 'xls'];

/**
 * Read a file and parse it into an array of row objects
...
/**
 * Write an array of row objects to a file in the given format
 * @param {Array<Object>} data
 * @param {string} outputPath
 * @param {{ minify?: boolean, delimiter?: string, sheet?: string }} [options]
 */
export async function writeFile(data, outputPath, options = {}) {
 ...
 * @param {string} targetFormat - e.g. 'json', 'csv', 'xlsx'
 * @param {{ output?: string, sheet?: string, minify?: boolean, delimiter?: string }} [options]
 * @returns {Promise<{ outputPath: string, rowCount: number }>}
 */
export async function convertFile(inputPath, targetFormat, options = {}) {
  ...
  if (inputExt === targetFormat || (inputExt === 'xls' && targetFormat === 'xlsx')) {
...
/**
 * Main convert function — handles single file or batch directory
 * @param {string} input - file path or directory
 * @param {{ to: string, output?: string, sheet?: string, minify: boolean, delimiter: string, dryRun: boolean }} options
 */
async function convert(input, options) {
  ...
  if (isDir) {
    ...
    const files = await glob(`${resolvedInput}/*.{${SUPPORTED_FORMATS.join(',')}}`, { nodir: true });
    ...
    if (dryRun) {
      toConvert.forEach(f =>
        console.log(`  ${chalk.gray(path.basename(f))} → ${chalk.cyan(`${path.basename(f, path.extname(f))}.${targetFormat}`)}`)
      );
    ...
    if (dryRun) {
      ...
      console.log(`📋 DRY RUN: ${chalk.gray(path.basename(input))} → ${chalk.cyan(`${baseName}.${targetFormat}`)}`);
      return;
    }
...
Examples:
  $ fileflow convert data.csv --to json
  $ fileflow convert data.xlsx --to csv --sheet "Sales"
  $ fileflow convert ./data-dir --to json`)
    .action(convert);
}
